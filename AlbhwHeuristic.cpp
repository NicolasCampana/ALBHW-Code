...
int AlbhwHeuristic::getWorkerValue(int workerId) {
    return this->instanceProblem->wCosts.at(workerId);
}
int AlbhwHeuristic::getNumberOfWorkers() {
    return this->instanceProblem->numW;
}
int AlbhwHeuristic::getNumberTasks() {
    return this->instanceProblem->numT;
}
...
AlbhwHeuristicResult* AlbhwHeuristic::Heuristic(int optRegra, int optTrab) {
    ...
    for(s = 0; (s < numS) and sizeOfGraph > 0; s++) {
        ...
        //// cout << "Quantidade de Tarefas Restantes: " << sizeOfGraph << endl;
...
// PORAAAAAAAAAAA REVER ISSO AQUI CARALHO
...
int AlbhwHeuristic::MaxTimeTrabWorker(int trab, std::vector<int> tasks) {
    ...
    int choosed = -1;
    for(int i = 0; i < int(tasks.size()); ++i) {
        if(this->instanceProblem->tTypes[tasks.at(i)] == trab and
           this->instanceProblem->twTimes[tasks.at(i)][trab] > bigger) { 
            bigger = this->instanceProblem->twTimes[tasks.at(i)][trab];
            choosed = tasks.at(i);
        } else if(this->instanceProblem->twTimes[tasks.at(i)][trab] == bigger) {
