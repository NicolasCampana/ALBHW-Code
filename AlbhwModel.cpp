...
int AlbhwModel::improveCycT(int **twTimes, int twTimesOrig, std::vector<int> tTypes, int cycT, int numT, int task, int w) {
    try {
        ...
        int Mh = int(model->get(GRB_DoubleAttr_ObjVal));
        return Mh;
    }  catch (GRBException e) {
        std::cout << "Error code = " << e.getErrorCode() << std::endl;
        std::cout << e.getMessage() << std::endl;
        ...
        for(h = 0; h < numW; ++h) {
            for(i = 0; i < numT; ++i) {
                ...
                if(ntwTimes[i][h] != twTimes[i][h]) {
                    cout << "Porra deu bom! " << melhorado << " - " << ntwTimes[i][h] << " " << twTimes[i][h] << endl;
                    char tt; cin >> tt;
                }
            }
        }  */
        ...
        for(s = 0; s < numS; ++s) {
            for(h = 0; h < numW; ++h) {
                if(y[h][s].get(GRB_DoubleAttr_X) > 0.5) {
                    totalStations++;
                    result->workersA.push_back(h);
                }
            }
        }
        result->workstationTime = new int[totalStations];
        result->tasksA = new vector<int>[totalStations];
        for(s = 0; s < totalStations; ++s) {
            result->workstationTime[s] = 0;
            for(i = 0; i < numT; ++i) {
                for(h = 0; h < numW; ++h) {
                    if(x[i][h][s].get(GRB_DoubleAttr_X) == 1) {
                        cout << "A tarefa " << i << " com o trab " << h << " na estação " << s << endl;
                        result->workstationTime[s] += this->graph->taskTimes[i][h];
                        result->tasksA[s].push_back(i);
                    }
                }
            }
            cout << "Workstation: " << result->workstationTime[s] << " Worker: " << result->workersA[s] << endl;
        }
        result->numS = totalStations;
        result->valorObjetivoModelo = int(model->get(GRB_DoubleAttr_ObjVal));
        ...
        for(s = 0; s < totalStations; ++s) {
            ...
            for(i = 0; i < numT; ++i) {
                for(h = 0; h < numW; ++h) {
                    if(x[i][h][s].get(GRB_DoubleAttr_X) == 1) {
                        cout << "A tarefa " << i << " com o trab " << h << " na estação " << s << endl;
                        result->workstationTime[s] += twTimes[i][h];
                        result->tasksA[s].push_back(i);
                    }
                }
            }
...
void AlbhwModel::printResultsHeur(AlbhwHeuristicResult* res, int regraT, int regraW, int print, string destination, string filename) {
    ...
    if(arq) {        
        ...
        cout << "Error writing the results from the heuristic.";
    }
    arq.close();
}
// Buscas Locais MIP
...
AlbhwHeuristicResult* AlbhwModel::RunVND(AlbhwHeuristicResult *heurRes, int regraT, int regraW) {
    ...
    while(i < 2) {
        if(i == 0) {
            cout << "Entrando MIP1"; //cin >> c;
            heurImproved = this->LocalSearchMIP1(bestN);
        } else {
            cout << "Entrando MIP2"; //cin >> c;
